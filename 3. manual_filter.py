...
def scanFolder(folder_names, search_dir):
    ...
    for folder_name in folder_names:
        ...
        tabControl = ttk.Notebook(root, width=colls*20)
        ...
        image_path_groups = [image_paths[x:x+total_images] for x in range(0, len(image_paths), total_images)]
        ...
        for image_path_group in image_path_groups:
            ...
            for x in range(len(image_array[tab_count])):
                ...
                for y in range(len(image_group)):
                    boxes[tab_count][x].append(Checkbutton(tab, variable=image_group[y]["var"], image = image_group[y]["image"], command = lambda x = x: checkRow(tab_count, x, y), background="grey"))
